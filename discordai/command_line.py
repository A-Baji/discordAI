...
def discordai():
    ...
    config_cmds = command.add_parser(
        "config", description="View and modify your config"
    )
    ...
    bot_cmds_commands_subcommand = bot_cmds_commands.add_subparsers(
        dest="subsubcommand"
    )

    new_cmd = bot_cmds_commands_subcommand.add_parser(
        "new",
        description="Create a new slash command for your bot that will use a customized model for completions",
    )
    new_cmd_required_named = new_cmd.add_argument_group("required named arguments")
    ...
    job_status_optional_named = job_status.add_argument_group(
        "optional named arguments"
    )
    ...
    job_cancel_optional_named = job_cancel.add_argument_group(
        "optional named arguments"
    )
    ...
    if args.command == "bot":
        ...
        if args.subcommand == "bot-token":
            if args.new_token:
                ...
                configuration.save(
                    json.dumps(
                        dict(token=args.new_token, openai_key=config["openai_key"])
                    )
                )
                config = configuration.get()
            ...
            if args.new_key:
                ...
                configuration.save(
                    json.dumps(dict(token=config["token"], openai_key=args.new_key))
                )
                config = configuration.get()
